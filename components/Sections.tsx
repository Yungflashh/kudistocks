...
function StatCard({ icon, value, description, delay = 0 }: { icon: ReactNode; value: ReactNode; description: string; delay?: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: delay / 1000 }}
      whileHover={{ y: -6, boxShadow: "0 20px 60px rgba(46,63,199,0.4)" }} className="relative">
      <div className="relative bg-[#2e3fc7] rounded-2xl p-6 md:p-8 overflow-hidden">
        <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full bg-white/5" />
        <div className="absolute -bottom-4 -left-4 w-16 h-16 rounded-full bg-white/5" />
        <div className="w-12 h-12 rounded-xl bg-white/15 flex items-center justify-center mb-6">{icon}</div>
        <p className="text-white text-4xl md:text-5xl font-extrabold tracking-tight leading-none">{value}</p>
        <p className="text-blue-100/70 text-sm mt-3 leading-relaxed">{description}</p>
      </div>
    </motion.div>
  );
}
...
export function AboutSection() {
  return (
    <section id="aboutus" className="relative bg-[#f5f6fa] text-gray-900 py-20 md:py-32 px-6 md:px-12 lg:px-20">
      <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: "radial-gradient(circle, #2e3fc7 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
...
// Feature mockups (inline)
function AIMock() { return (<div className="w-full px-4"><div className="bg-gray-50 rounded-t-2xl border border-gray-100 border-b-0 p-4"><div className="flex justify-center mb-3"><div className="w-36 h-36 rounded-full relative overflow-hidden" style={{background:"linear-gradient(135deg, #60a5fa 0%, #34d399 40%, #fbbf24 70%, #f87171 100%)"}}><div className="absolute inset-0 bg-gradient-to-t from-white/20 to-transparent" /><div className="absolute top-6 left-8 w-12 h-8 bg-green-500/60 rounded-full blur-sm" /><div className="absolute top-10 right-6 w-8 h-10 bg-green-600/50 rounded-full blur-sm" />{[0,45,90,135,180,225,270,315].map((a,i)=>{const r=76,x=72+r*Math.cos((a*Math.PI)/180),y=72+r*Math.sin((a*Math.PI)/180);const c=["bg-blue-400","bg-purple-400","bg-pink-400","bg-indigo-400","bg-teal-400","bg-orange-400","bg-rose-400","bg-cyan-400"];return <div key={i} className={`absolute w-6 h-6 rounded-full ${c[i]} border-2 border-white shadow-sm`} style={{left:x-12,top:y-12}} />;})}</div></div><div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100"><p className="text-xs font-semibold text-gray-700">How can I assist you today?</p><p className="text-[10px] text-gray-400 mt-1">AI-powered insights for forecasting and expense optimization.</p></div></div></div>); }
function InvMock() { return (<div className="w-full px-4"><div className="bg-gray-50 rounded-t-2xl border border-gray-100 border-b-0 p-4"><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><div className="w-6 h-6 bg-blue-100 rounded-lg flex items-center justify-center"><svg viewBox="0 0 16 16" fill="#3b82f6" className="w-3 h-3"><rect x="1" y="8" width="3" height="7" rx="0.5"/><rect x="6" y="4" width="3" height="11" rx="0.5"/><rect x="11" y="1" width="3" height="14" rx="0.5"/></svg></div><span className="text-xs font-semibold text-gray-800">Analytics</span></div><span className="text-[10px] text-gray-400">Elegance salon</span></div><svg viewBox="0 0 240 60" className="w-full h-14 mb-3"><polyline points="0,50 30,42 60,45 90,30 120,35 150,20 180,25 210,12 240,8" fill="none" stroke="#3b82f6" strokeWidth="2.5" strokeLinecap="round" /><polyline points="0,50 30,42 60,45 90,30 120,35 150,20 180,25 210,12 240,8 240,60 0,60" fill="url(#iGS)" stroke="none" /><defs><linearGradient id="iGS" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3b82f6" stopOpacity="0.15" /><stop offset="100%" stopColor="#3b82f6" stopOpacity="0" /></linearGradient></defs></svg><div className="flex gap-2"><div className="flex-1 bg-white rounded-lg p-2 border border-gray-100"><p className="text-[9px] text-gray-400">Large cream</p><p className="text-xs font-bold text-gray-900">₦1,600.00</p></div><div className="flex-1 bg-white rounded-lg p-2 border border-gray-100"><p className="text-[9px] text-gray-400">Revenue</p><p className="text-xs font-bold text-gray-900">₦1,600,000.32</p></div></div></div></div>); }
function LangMock() { return (<div className="w-full px-4"><div className="bg-gray-50 rounded-t-2xl border border-gray-100 border-b-0 p-4"><div className="flex flex-wrap gap-2 mb-3">{["🇬🇧 English","🇳🇬 Yoruba","🇳🇬 Igbo","🇳🇬 Hausa","🇫🇷 French","🇪🇸 Spanish"].map((l,i)=>(<div key={i} className={`px-3 py-1.5 rounded-full text-[10px] font-medium border ${i===0?"bg-blue-500 text-white border-blue-500":"bg-white text-gray-600 border-gray-200"}`}>{l}</div>))}</div><div className="bg-white rounded-xl p-3 border border-gray-100 space-y-2"><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center text-sm">🌍</div><div><p className="text-xs font-semibold text-gray-800">Multi-Language</p><p className="text-[10px] text-gray-400">Ọjà rẹ ni ede rẹ</p></div></div><div className="h-px bg-gray-100" /><p className="text-[10px] text-gray-500">Manage your business in the language you understand best.</p></div></div></div>); }
function OrdMock() { return (<div className="w-full px-4"><div className="bg-gray-50 rounded-t-2xl border border-gray-100 border-b-0 p-4"><div className="space-y-2.5">{[{s:"Pending",i:"⏳",c:"bg-yellow-50 text-yellow-700 border-yellow-200"},{s:"Confirmed",i:"✓",c:"bg-blue-50 text-blue-700 border-blue-200"},{s:"In Transit",i:"🚚",c:"bg-purple-50 text-purple-700 border-purple-200"},{s:"Delivered",i:"✅",c:"bg-green-50 text-green-700 border-green-200"}].map((x,i)=>(<div key={i} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border ${x.c}`}><span className="text-sm">{x.i}</span><span className="text-xs font-medium flex-1">{x.s}</span><svg viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3 opacity-40"><path d="M6 3l5 5-5 5V3z"/></svg></div>))}</div></div></div>); }
function FinMock() { return (<div className="w-full px-4"><div className="bg-gray-50 rounded-t-2xl border border-gray-100 border-b-0 p-4"><div className="space-y-2 mb-3"><div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-3 text-white"><p className="text-[10px] opacity-70">Total Balance</p><p className="text-lg font-bold">₦978,675.65</p></div><div className="flex gap-2"><div className="flex-1 bg-white rounded-lg p-2 border border-gray-100"><p className="text-[9px] text-gray-400">Income</p><p className="text-xs font-bold text-green-600">₦600,765.23</p></div><div className="flex-1 bg-white rounded-lg p-2 border border-gray-100"><p className="text-[9px] text-gray-400">Expenses</p><p className="text-xs font-bold text-red-500">₦1,892,920.05</p></div></div></div><div className="flex items-end gap-1 h-12">{[40,65,35,80,50,70,55,90,45,60,75,85].map((h,i)=>(<div key={i} className="flex-1 rounded-t-sm" style={{height:`${h}%`,background:i%2===0?"#3b82f6":"#93c5fd"}} />))}</div></div></div>); }
function MsMock() { return (<div className="w-full px-4"><div className="bg-gray-50 rounded-t-2xl border border-gray-100 border-b-0 p-4"><div className="space-y-2 mb-3">{[{n:"MamaTola Stores",l:"Lagos, NG",s:"Active",c:"bg-green-100 text-green-700"},{n:"Branch 2 — Abuja",l:"Abuja, NG",s:"Active",c:"bg-green-100 text-green-700"},{n:"Branch 3 — PH",l:"Port Harcourt",s:"Syncing",c:"bg-blue-100 text-blue-700"}].map((st,i)=>(<div key={i} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-gray-100"><div className="w-9 h-9 rounded-xl bg-blue-500 flex items-center justify-center text-white text-xs font-bold">{st.n.charAt(0)}</div><div className="flex-1"><p className="text-xs font-semibold text-gray-800">{st.n}</p><p className="text-[10px] text-gray-400">{st.l}</p></div><span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${st.c}`}>{st.s}</span></div>))}</div><div className="flex justify-center"><div className="w-14 h-24 bg-gradient-to-b from-blue-600 to-indigo-700 rounded-2xl border-2 border-gray-300 flex items-center justify-center shadow-lg"><svg viewBox="0 0 24 24" fill="white" className="w-6 h-6"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.53 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.32 2.32-1.55 4.24-3.74 4.25z"/></svg></div></div></div></div>); }
    ...
    <section id="features" className="relative bg-white text-gray-900 py-20 md:py-32 px-6 md:px-12 lg:px-20">
      <div className="relative max-w-7xl mx-auto">
        <div className="text-center mb-16 md:mb-20">
          <ScrollFadeUp><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-100 text-blue-600 text-sm font-medium mb-6">⚡ Our features</div></ScrollFadeUp>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FeatureCard title="AI-Powered Insights" description="Real-time smart analytics for forecasting, expense optimization, and supplier performance advice." delay={0}><AIMock /></FeatureCard>
          <FeatureCard title="Inventory Management" description="Monitor stock with pinpoint accuracy, restock alerts, and informed restocking decisions." delay={150}><InvMock /></FeatureCard>
          <FeatureCard title="Multi – Language Support" description="Use KudiStocks in multiple languages for smoother business management." delay={0}><LangMock /></FeatureCard>
          <FeatureCard title="Order Management" description="Create and track purchase orders with ease. Keep inventory organized." delay={150}><OrdMock /></FeatureCard>
          <FeatureCard title="Financial Management" description="Full clarity into financial insights with accurate records and real-time cash flow." delay={0}><FinMock /></FeatureCard>
          <FeatureCard title="Multi – Store Support" description="Manage multiple stores under one account, monitoring each independently." delay={150}><MsMock /></FeatureCard>
        </div>
      </div>
    </section>
  );
...
export function HowItWorksSection() {
  ...
  return (
    <section className="relative bg-[#f5f6fa] text-gray-900 py-20 md:py-32 px-6 md:px-12 lg:px-20">
      <div className="relative max-w-7xl mx-auto">
        <div className="text-center mb-16 md:mb-20">
          <ScrollFadeUp><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-100 text-blue-600 text-sm font-medium mb-6">⚡ How It Works</div></ScrollFadeUp>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
          {steps.map((s, i) => (
            <ScrollFadeUp key={i} delay={i * 200}>
              <div className="mb-6"><h3 className="text-lg font-bold text-gray-900 mb-2"><span className="text-gray-400 mr-1">{s.n}.</span> {s.t}</h3><p className="text-gray-500 text-sm leading-relaxed">{s.d}</p></div>
              <motion.div whileHover={{ y: -8 }} className="flex justify-center">{s.phone}</motion.div>
            </ScrollFadeUp>
          ))}
        </div>
      </div>
...
export function CTASection() {
  return (
    <section className="relative py-20 md:py-32 px-6 md:px-12 lg:px-20 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-[#0a0e27] via-[#1a237e] to-[#0d1442]" /><div className="absolute inset-0 grid-bg opacity-50" /><GlowOrb className="w-80 h-80 bg-blue-500 top-0 right-1/4" delay={1} /><GlowOrb className="w-64 h-64 bg-indigo-400 bottom-0 left-1/4" delay={3} />
      <div className="relative max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
        <div className="lg:w-3/5 text-center lg:text-left">
          <ScrollFadeUp><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 text-sm text-blue-200 mb-6">🚀 100,000+ businesses trust KudiStocks</div></ScrollFadeUp>
          <ScrollFadeUp delay={100}><h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold leading-[1.1] text-white max-w-2xl">Make Smarter Business Decisions, Just Like Other Business Owners</h2></ScrollFadeUp>
          <ScrollFadeUp delay={200}><p className="mt-5 text-blue-200/70 text-base md:text-lg max-w-xl">Join over 100,000 active users managing inventory, orders, and sales with KudiStocks every day.</p></ScrollFadeUp>
          <ScrollFadeUp delay={300}><div className="flex flex-wrap gap-4 mt-8 justify-center lg:justify-start"><motion.button whileHover={{ y: -2 }} className="flex items-center gap-2 px-6 py-3 rounded-full font-medium text-sm bg-blue-500 hover:bg-blue-400 text-white">Download on iOS <span className="text-lg">🍎</span></motion.button><motion.button whileHover={{ y: -2 }} className="flex items-center gap-2 px-6 py-3 rounded-full font-medium text-sm border-2 border-blue-400 text-blue-200 hover:bg-blue-400/10">Download on Android <span className="text-lg">🤖</span></motion.button></div></ScrollFadeUp>
          <ScrollFadeUp delay={400}><div className="flex flex-wrap gap-3 mt-8 justify-center lg:justify-start">{[{i:"✨",t:"Inventory Tracking"},{i:"🔒",t:"Password Security"},{i:"📊",t:"Smarter Stock Decision"},{i:"📋",t:"Organized Orders & Sales"}].map((p,i)=>(<div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs text-blue-100"><span>{p.i}</span>{p.t}</div>))}</div></ScrollFadeUp>
        </div>
        <div className="lg:w-2/5 flex justify-center"><ScrollFadeUp delay={300}><div className="w-72 h-96 rounded-3xl overflow-hidden relative bg-gradient-to-b from-blue-400/10 to-indigo-600/20 border border-white/10"><div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48"><div className="w-20 h-20 rounded-full bg-gradient-to-br from-amber-200 to-amber-400 mx-auto mb-2" /><div className="w-40 h-52 bg-gradient-to-b from-blue-300 to-blue-500 rounded-t-3xl mx-auto relative overflow-hidden"><div className="absolute top-4 left-1/2 -translate-x-1/2"><div className="w-16 h-28 bg-gray-900 rounded-xl border-2 border-gray-600 shadow-lg"><div className="w-10 h-1 bg-black rounded-full mx-auto mt-1" /><div className="mx-1 mt-1 bg-blue-500/30 h-5 rounded" /><div className="mx-1 mt-0.5 bg-green-500/30 h-3 rounded" /><div className="mx-1 mt-0.5 bg-white/10 h-2 rounded" /></div></div></div></div><div className="absolute top-8 right-4 bg-white/10 backdrop-blur rounded-lg px-2 py-1 text-[10px] text-white border border-white/10">📈 +24%</div><div className="absolute top-20 left-4 bg-white/10 backdrop-blur rounded-lg px-2 py-1 text-[10px] text-white border border-white/10">📦 Stock OK</div><div className="absolute top-36 right-6 bg-green-500/20 backdrop-blur rounded-lg px-2 py-1 text-[10px] text-green-300 border border-green-500/20">✅ Synced</div></div></ScrollFadeUp></div>
      </div>
    </section>
  );
}
    ...
    <footer className="relative bg-[#060a1e] text-gray-400 pt-16 pb-12 px-6 md:px-12 lg:px-20 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="border-t border-gray-800/50 pt-8">
          {/* Copyright */}
          <p className="text-gray-600 text-sm mb-8">© 2025. All rights reserved</p>

          {/* Large logo */}
          <div className="flex items-center gap-4">
            {/* Logo icon - stylized pages/leaves */}
            <svg viewBox="0 0 60 60" className="w-16 h-16 text-gray-700" fill="currentColor">
              <path d="M15 45c0-16.569 13.431-30 30-30v6c-13.255 0-24 10.745-24 24h-6z" opacity="0.4" />
              <path d="M22 45c0-12.703 10.297-23 23-23v6c-9.389 0-17 7.611-17 17h-6z" opacity="0.6" />
              <path d="M29 45c0-8.837 7.163-16 16-16v6c-5.523 0-10 4.477-10 10h-6z" opacity="0.8" />
            </svg>
            <span className="text-gray-700 text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-tight">
              Kudistocks.
            </span>
          </div>
        </div>
